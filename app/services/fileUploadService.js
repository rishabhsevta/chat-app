...
fileUploadService.uploadFileToLocal = async (payload, fileName, pathToUpload, pathOnServer) => {
    let directoryPath = pathToUpload ? pathToUpload : path.join(__dirname + `../../..${CONFIG.PATH_TO_UPLOAD_FILES_ON_LOCAL}`);
    // create user's directory if not present.
    if (!fs.existsSync(directoryPath)) {
        fs.mkdirSync(directoryPath);
    }
    payload.file.originalname=Date.now()+payload.file.originalname;
    let fileSavePath = `${directoryPath}/${payload.file.originalname}`;
    ...
    return new Promise((resolve, reject) => {
        ...
        writeStream.end(function (err) {
            if (err) {
                ...
                let fileUrl = pathToUpload ? `${CONFIG.SERVER_URL}${pathOnServer}/${payload.file.originalname}` : `${CONFIG.SERVER_URL}${CONFIG.PATH_TO_UPLOAD_FILES_ON_LOCAL}/${payload.file.originalname}`;
                let filePath = url.parse(fileUrl,true);
                resolve(filePath.pathname);
            }
        });
    });
};
